<x-stories-layout>
    <div class="space-y-6 sm:space-y-8 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        @if($stories->isEmpty())
            <p class="text-stone-600 text-sm">Nėra istorijų.</p>
        @else
            <div class="flex flex-col sm:flex-row sm:justify-end mb-4 sm:mb-6">
                <form method="GET" class="w-full sm:w-auto">
                    <select
                        name="sort"
                        class="w-full sm:w-auto px-3 py-2 rounded-lg border border-stone-300 bg-white text-sm shadow-sm
                            focus:ring-2 focus:ring-stone-500 transition cursor-pointer"
            ...
            <div class="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($stories as $story)
                    <a href="{{ route('stories.show', $story) }}" class="group block bg-white border border-stone-200 rounded-2xl
                    overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                        @if(!empty($story->cover_image_path))
                            ...
                            <img
                                ...
                                class="w-full aspect-[16/10] object-cover transition duration-300 group-hover:brightness-95"
                            >
                        @else
                        ...
                        <div class="p-4 sm:p-5">
                            <h3 class="font-serif text-lg sm:text-xl font-semibold group-hover:underline">
                                {{ $story->title }}
                            </h3>
