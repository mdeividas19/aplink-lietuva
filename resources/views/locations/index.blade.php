        ...
        <div class="py-8 sm:py-12 bg-gray-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-8">
                    ...
                    <div class="p-5 sm:p-8 text-gray-900">

                        <div class="mt-4 sm:mt-6 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">

                            <input type="text" id="locationSearch"
                                   placeholder="Ieškoti vietovės..."
                                   class="w-full sm:flex-1 px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-forest-green-500 focus:border-transparent text-gray-900 bg-white text-sm sm:text-base">

                            <select id="cityFilter"
                                    class="w-full sm:w-64 px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-forest-green-500 focus:border-transparent text-gray-900 bg-white text-sm sm:text-base">
                                <option value="">-</option>
                    ...
                    @can('isAdmin')
                        <div class="px-5 sm:px-8 pb-5 sm:pb-6">
                            <a href="{{ route('locations.create') }}"
                               class="inline-flex items-center justify-center w-full sm:w-auto px-6 py-3 bg-amber-500 text-white rounded-lg font-semibold
          hover:bg-amber-600 transition-all duration-200 shadow-md hover:shadow-lg text-sm sm:text-base">
                                + Nauja vieta
                            </a>
                        </div>
                    @endcan
                ...
                <div class="space-y-10 sm:space-y-12" id="locationsContainer">
                    @foreach($grouped as $letter => $locations)
                        <div class="letter-section" data-letter="{{ $letter }}">
                            <h2 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-forest-green-700 border-b-2 border-amber-400 pb-1.5 sm:pb-2 inline-block">
                                {{ $letter }}
                            </h2>
                 ...
                 8.688 11.25s8.688-4.03 8.688-11.25z"
                                        ...
                                        <a href="{{ route('locations.show', $location->id) }}"
                                           ...
                                           data-city="{{ $location->city_id }}">
                                            ...
                                            @endif
                                            <div class="px-4 py-3 sm:py-4 bg-white">
                                        ...
                                        @can('isAdmin')
                                            <div class="flex flex-col sm:flex-row gap-2 mt-3 px-2">
                                                <a href="{{ route('locations.edit', $location->id) }}"
                                                   class="flex-1 inline-flex items-center justify-center px-4 py-2 bg-forest-green-600 text-white rounded-lg font-semibold text-xs sm:text-sm
          hover:bg-forest-green-700 transition-all duration-200">
                                                ...
                                                <form action="{{ route('locations.destroy', $location->id) }}" method="POST" onsubmit="return confirm('Are you sure?');" class="flex-1">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 bg-red-600 text-white rounded-lg font-semibold text-xs sm:text-sm
