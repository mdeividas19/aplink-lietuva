    ...
    <div class="py-8 sm:py-12 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 flex flex-col md:flex-row flex-wrap items-stretch md:items-center justify-between gap-3 sm:gap-4">
                    <a href="{{ url()->previous() }}"
                       class="inline-flex items-center justify-center px-3 sm:px-4 py-2 bg-forest-green-600 hover:bg-forest-green-700 text-white font-semibold rounded-lg shadow transition-colors duration-200 text-sm sm:text-base">
                 ...
                 8.688 11.25s8.688-4.03 8.688-11.25z"
                        ...
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ $url }}"
                           ...
                           class="inline-flex items-center gap-2 px-4 py-2.5 w-full sm:w-auto justify-center bg-[#1877f2] hover:bg-[#166fe5] text-white font-medium text-sm rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                        ...
                        <a href="https://twitter.com/intent/tweet?url={{ $url }}&text={{ $text }}"
                           target="_blank" rel="noopener"
                           class="inline-flex items-center gap-2 px-4 py-2.5 w-full sm:w-auto justify-center bg-black hover:bg-gray-900 text-white font-medium text-sm rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                <div class="px-4 sm:px-8 pt-6 text-center">
                    <h1 class="text-3xl sm:text-4xl md:text-5xl font-serif mb-2 text-forest-green-700">
                        {{ $location->name }}
                    </h1>
                </div>
                ...
                <div class="px-4 sm:px-8 mt-4">
                    @if ($location->images && $location->images->count())
                        <div class="relative rounded-lg overflow-hidden shadow-lg">
                            <img
                                class="w-full h-60 sm:h-72 md:h-96 object-cover"
                ...
                @if ($location->images->count() > 1)
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 px-4 sm:px-8 mt-6">
                        @foreach($visibleImages as $index => $image)
                            <div class="relative overflow-hidden rounded-lg shadow-md cursor-pointer group hover:shadow-xl transition-shadow z-40"
                                 onclick="openGallery({{ $index - 1 }})">
                                <img
                                    class="w-full h-32 sm:h-40 md:h-48 object-cover group-hover:scale-110 transition-transform duration-300"
                        ...
                        @if($remainingCount > 0)
                            ...
                            <div class="relative overflow-hidden rounded-lg shadow-md cursor-pointer group hover:shadow-xl transition-shadow z-40"
                                ...
                                <img
                                    ...
                                    alt="More images"
                                >
                                <div class="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center text-white text-2xl sm:text-3xl font-bold">
                                    +{{ $remainingCount - 1 }}
                                </div>
                            </div>
                        @endif
                ...
                <div id="galleryModal" class="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 hidden">
                    <button class="absolute top-5 right-5 text-white hover:text-gray-300 text-3xl sm:text-4xl font-bold" onclick="closeGallery()">×</button>
                    <div class="relative w-[94vw] sm:w-[90vw] max-w-6xl h-[75vh] sm:h-[80vh]">
                        <img id="galleryImage" class="w-full h-full object-contain rounded shadow-lg">
                        <button class="absolute left-0 top-1/2 -translate-y-1/2 bg-white bg-opacity-20 hover:bg-opacity-30 text-white text-3xl sm:text-4xl px-4 sm:px-6 py-3 sm:py-4 rounded-r-lg" onclick="prevImage()">‹</button>
                        <button class="absolute right-0 top-1/2 -translate-y-1/2 bg-white bg-opacity-20 hover:bg-opacity-30 text-white text-3xl sm:text-4xl px-4 sm:px-6 py-3 sm:py-4 rounded-l-lg" onclick="nextImage()">›</button>
                    </div>
                </div>

                <div class="mt-10 px-4 sm:px-8 pb-8">
                    @if ($location->description)
                        <h2 class="text-2xl sm:text-3xl font-serif mb-4 sm:mb-6 text-forest-green-700 text-center">
                            Aprašymas
                        </h2>
                    ...
                    @if ($location->address || $location->phone_number)
                        <div class="mb-10 bg-gray-50 rounded-lg p-5 sm:p-6">
                            <h2 class="text-2xl sm:text-3xl font-serif mb-6 text-forest-green-700 text-center">
                                Kontaktai
                            </h2>
                                    ...
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <a href="tel:{{ $location->phone_number }}"
                                               class="text-forest-green-600 hover:text-forest-green-700 hover:underline">
                    ...
                    @if ($location->city && $location->address && $location->longitude && $location->latitude)
                        <div class="text-3xl p-4 sm:p-7 text-gray-900 text-center dark:text-gray-100">
                            <h1 class="text-3xl sm:text-4xl font-serif inline-block bg-white text-gray-900 w-full max-w-xs py-1 rounded-full shadow-md">
                                Žemėlapis
                            </h1>
                        </div>
                        <div class="mb-7 px-4 sm:mx-7 mt-2 text-gray-900 dark:text-gray-100">
                            <div>
                    ...
                    <div class="mt-8">
                        <h2 class="text-xl sm:text-2xl font-semibold mb-4">Atsiliepimai</h2>
