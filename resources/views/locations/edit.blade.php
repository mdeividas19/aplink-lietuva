        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <h2 class="text-3xl font-serif text-center mb-6">Redaguoti vietovę</h2>
                    ...
                    <form id="update-location-form" method="POST" action="{{ route('locations.update', $location->id) }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                      class="mt-1 w-full px-3 py-2 rounded-md border text-gray-900">{{ old('description', $location->description) }}</textarea>
                        </div>
                        ...
                        @if ($location->firstImage)
                            <div class="text-center">
                                <h2 class="text-2xl mb-2">Pagrindinė nuotrauka</h2>
                                <img id="current-first-image" src="{{ asset('storage/location_images/' . basename($location->firstImage->image_path)) }}"
                                     class="w-full max-h-64 object-cover rounded-md mt-2 border" alt="Nuotraukos nėra">
                            </div>
                        @endif
                    </form>
                    ...
                    <div class="text-center my-6">
                        ...
                        <div id="extra-photos" data-location-id="{{ $location->id }}" class="flex flex-wrap gap-4 mt-4">
                            @foreach ($location->images->skip(1) as $image)
                                <div class="relative w-52 h-48 rounded overflow-hidden border">
                        ...
                        <form id="add-photo-form" enctype="multipart/form-data" class="flex flex-col items-center gap-2">
                            @csrf
                            <label class="cursor-pointer px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">
                                Pridėti nuotrauką
                                <input type="file" name="image" accept="image/*"  class="hidden"
                    ...
                    <div class="flex justify-between items-center mt-6">
                        <a href="{{ route('locations.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">
                            Atgal
                        </a>
                        <button form="update-location-form" type="submit" class="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700">
                            Atnaujinti vietovę
                        </button>
                    </div>
